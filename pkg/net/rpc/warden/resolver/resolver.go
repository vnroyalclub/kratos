...
func (b *Builder) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	if target.Scheme == kubernetes.KubernetesSchema {
		ti := target.Scheme + "," + target.Authority + "," + target.Endpoint
		r = &Resolver{
			nr:   b.Builder.Build(ti),
			cc:   cc,
...
func (r *Resolver) ResolveNow(o resolver.ResolveNowOptions) {
	//log.Info("GRPC ResolveNow")
	//if State, ok := r.nr.Fetch(context.Background()); ok {
	//	r.updateState(State)
	//}
}
...
func (r *Resolver) updateState(State resolver.State) {
	//更新State 写入addrs列表以及LoadBalancer
	r.cc.UpdateState(State)
	log.Info("GRPC State Update addr: %v", State.Addresses)
}

//重写new addr 为 UpdateState
