...
func AddFlag(fs *flag.FlagSet) {
	...
	fs.StringVar(&httpDSN, "bm.http", v, "listen http dsn, or use HTTP env variable.")
}
...
func NewServer(conf *config.ServerConfig) *Engine {
	if conf == nil {
		if !flag.Parsed() {
	...
	engine.NoRoute(func(c *Context) {
		c.Bytes(404, "text/plain", config.Default404Body)
		c.Abort()
	})
	engine.NoMethod(func(c *Context) {
		c.Bytes(405, "text/plain", config.Default405Body)
		c.Abort()
	})
	startPerf(engine)
	return engine
...
func DefaultServer(conf *config.ServerConfig) *Engine {
...
func (engine *Engine) SetConfig(conf *config.ServerConfig) (err error) {
	if conf.Timeout <= 0 {
