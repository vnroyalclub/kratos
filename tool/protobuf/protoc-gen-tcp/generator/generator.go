...
// TcpGenerator tcp generator
func TcpGenerator() *tcp {
	t := &tcp{}
	return t
}

// Generate ...
func (t *tcp) Generate(in *plugin.CodeGeneratorRequest) *plugin.CodeGeneratorResponse {
	t.Setup(in)
...
func (t *tcp) generateForFile(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
	...
	for i, service := range file.Service {
		count += t.generateTcpInterface(file, service)
		t.generateTcpService(file, service, i)
	}
	resp.Name = proto.String(naming.GenFileName(file, ".tcp.go"))
...
func (t *tcp) generateFileHeader(file *descriptor.FileDescriptorProto, pkgName string) {
	t.P("// Code generated by protoc-gen-tcp ", generator.Version, ", DO NOT EDIT.")
	...
	if t.filesHandled == 0 {
		...
		if err == nil && comment.Leading != "" {
			...
			t.P("/*")
			t.P("Package ", t.GenPkgName, " is a generated tcp stub package.")
			t.P("This code was generated with kratos/tool/protobuf/protoc-gen-tcp ", generator.Version, ".")
			t.P()
...
func (t *tcp) generateImports(file *descriptor.FileDescriptorProto) {
	...
	t.P(`	"git.huoys.com/middle-end/kratos/pkg/log"`)
	t.P(`	"runtime/debug"`)
...
func (t *tcp) generateTcpInterface(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) int {
	count := 0
	servName := naming.ServiceName(service)
	t.P("// " + servName + "TcpServer is the server API for " + servName + " service.")
	...
	t.P(`SetCometChan(cl *tcp.ChanList, cs *tcp.Server)`)
...
func (t *tcp) generateInterfaceMethod(file *descriptor.FileDescriptorProto,
...
func (t *tcp) generateTcpService(
	...
	// Server registration.
	var TcpFuncName = fmt.Sprintf("Register%sCometServer", servName)
	t.P(`func `, TcpFuncName, `(s *tcp.Server, srv `, servName, `CometServer) {`)
...
func (t *tcp) generateServerMethod(servName, fullServName string, method *descriptor.MethodDescriptorProto) string {
	methName := naming.MethodName(method)
	hname := fmt.Sprintf("_Comet_%s_%s_Handler", servName, methName)
	inputType := t.GoTypeName(method.GetInputType())
	outputType := t.GoTypeName(method.GetOutputType())
	t.P(`func `, hname, `(srv interface{}, ctx context.Context, data []byte, interceptor tcp.UnaryServerInterceptor) ([]byte, error) {`)
